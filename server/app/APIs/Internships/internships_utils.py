...
from ...Models.model import Calendar, StudentSkills, Internship, City, Company, Comment, User, Skill, InternshipStatus, \
    Student
...
class InternshipsUtils:
 ...
 \
    ...
    def getRecommend(arg):
        ...
        if type == 'recommend':
            internships = db.session.query(Internship).join(StudentSkills,
                                                            Internship.skills.any(StudentSkills.skill_id)).filter(
                StudentSkills.student_id == 1)
        elif type == 'closing':
            internships = db.session.query(Internship).join(StudentSkills,
                                                            Internship.skills.any(StudentSkills.skill_id)).filter(
                StudentSkills.student_id == 1).order_by(Internship.expiration_datetime_utc == None,
                                                        Internship.expiration_datetime_utc.asc())
        elif type == 'new':
            internships = db.session.query(Internship).join(StudentSkills,
                                                            Internship.skills.any(StudentSkills.skill_id)).filter(
                StudentSkills.student_id == 1).order_by((Internship.posted_time.desc()))
