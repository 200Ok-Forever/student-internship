...
export const getRecommendations = async (id, token) => (
  await getRequest(`/company/${id}/recommendation`, { headers: { 'Authorization': `Bearer ${token}` }})
)

export const postInternship = async (id, data, token) => (
  await postRequest(`/company/${id}/create-job`, data, { headers: { 'Authorization': `Bearer ${token}` }})
)

export const putInternship = async (id, data, token) => (
  await putRequest(`/company/${id}/edit`, data, { headers: { 'Authorization': `Bearer ${token}` }})
)

export const deleteInternship = async (id, token) => (
  await deleteRequest(`/company/jobs/${id}`, { headers: { 'Authorization': `Bearer ${token}` }})
)