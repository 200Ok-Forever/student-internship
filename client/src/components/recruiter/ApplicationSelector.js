...
import { InputLabel, Select, FormControl, MenuItem, List, ListItem, ListItemButton, Avatar, Box, Typography } from '@mui/material';
    ...
    <Box>
      ...
      <List sx={{ mt: 2, maxHeight: '65vh', overflow: 'auto' }}>
        {filteredApplications.map(app => (
          <ApplicationCard app={app} setSelectedApp={setSelectedApp} selectedApp={selectedApp} />
        ))}
      </List>
    </Box>
  )
...
const ApplicationCard = ({ app, setSelectedApp, selectedApp }) => (
  <ListItem sx={{ p: 0, m: 0 }} className={selectedApp.applicationId === app.applicationId && classes.selected}>
    <ListItemButton 
      sx={{ p: 5 }} 
...
const ShortList = () => {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, color: theme.palette.warning.main }} mt={1}>
      <StarIcon />
      <Typography variant="subtitle1" sx={{ fontWeight: 'bold' }}>
        Shortlisted
      </Typography>
